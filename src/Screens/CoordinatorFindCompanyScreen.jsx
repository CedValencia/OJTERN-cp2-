...
const ReportModal = ({ company, onClose, onSubmit, reporter }) => {
  ...
  const handleFile = (e) => {
    ...
    if (!allowed.includes(file.type)) { setAlertMsg("Only PNG and PDF files are allowed."); return; }
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "16px" }}>
      <div style={{ background: "white", borderRadius: "16px", width: "100%", maxWidth: "520px", maxHeight: "85vh", overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ background: darkRed, padding: "12px 20px", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "6px" }}>
          ...
          {step < 3 ? (
            <button
              onClick={() => { if (step === 1 && !selected) { setAlertMsg("Please select a concern."); return; } setStep(step + 1); }}
...
const CompanyCard = ({ company, onViewProfile }) => {
  ...
  return (
    ...
    >
      ...
      <p style={{ fontFamily: "'Kufam', sans-serif", fontSize: "0.8rem", color: "#555", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>Location: {displayLocation}</p>
