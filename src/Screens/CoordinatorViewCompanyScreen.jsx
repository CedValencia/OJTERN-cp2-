    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#f5f5f5", overflow: "hidden", position: "relative" }}>
      <div className="coord-profile-content" style={{ flex: 1, overflowY: "auto" }}>
        ...
        <div className="coord-profile-top">
          ...
          <div className="coord-map-box" style={{ borderRadius: "14px", overflow: "hidden", background: "#d0d8e0", minHeight: "130px", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "6px" }}>
...
// ── Filter Panel ───────────────────────────────────────────────────────────────
const FilterPanel = ({ selectedIndustries, setSelectedIndustries, selectedRegion, setSelectedRegion, selectedProvince, setSelectedProvince, selectedCity, setSelectedCity, selectedBarangay, setSelectedBarangay }) => {
  ...
  const locationLevel = !selectedRegion ? "region" : !selectedProvince ? "province" : !selectedCity ? "city" : "barangay";
      ...
      <div style={{ padding: "4px 12px 10px" }}>
        ...
        {locationLevel === "region" && (
          <div style={{ maxHeight: "160px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "3px" }}>
            {REGIONS.length > 0 ? (
              REGIONS.map(r => (
                <div key={r.name} onClick={() => { setSelectedRegion(r.name); setSelectedProvince(""); setSelectedCity(""); setSelectedBarangay(""); }}
        ...
        {locationLevel === "province" && (
          <div>
            <BackLink label={selectedRegion} onClick={() => { setSelectedRegion(""); setSelectedProvince(""); setSelectedCity(""); setSelectedBarangay(""); }} />
            <div style={{ maxHeight: "150px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "3px" }}>
              {regionData?.provinces.map(p => (
                <div key={p.name} onClick={() => { setSelectedProvince(p.name); setSelectedCity(""); setSelectedBarangay(""); }}
                  style={{ padding: "4px 8px", borderRadius: "6px", fontSize: "0.72rem", cursor: "pointer", background: "#f7f0f0", color: darkRed, border: "1px solid #e0c0c0" }}
                  onMouseEnter={e => e.currentTarget.style.background = "#f0d0d0"}
                  onMouseLeave={e => e.currentTarget.style.background = "#f7f0f0"}
                >{p.name}</div>
        ...
        {locationLevel === "city" && (
          <div>
            <BackLink label={selectedProvince} onClick={() => { setSelectedProvince(""); setSelectedCity(""); setSelectedBarangay(""); }} />
            <div style={{ maxHeight: "150px", overflowY: "auto", display: "flex", flexWrap: "wrap", gap: "5px" }}>
              {provinceData?.cities.map(c => (
                <span key={c.name} onClick={() => { setSelectedCity(c.name); setSelectedBarangay(""); }}
        ...
        {locationLevel === "barangay" && (
          <div>
            <BackLink label={selectedCity} onClick={() => { setSelectedCity(""); setSelectedBarangay(""); }} />
            <div style={{ maxHeight: "150px", overflowY: "auto", display: "flex", flexWrap: "wrap", gap: "5px" }}>
...
const CoordinatorViewCompanyScreen = ({ onReportSubmit, onNavigateToReports, onMessageNow, initialCompanyId, onClearInitialCompany, onVisitCompany }) => {
  ...
  const filtered = companies.filter(c => {
    ...
    const matchCity     = !selectedCity     || (c.location?.city || c.location) === selectedCity;
  ...
  const clearAllFilters = () => { setSelectedIndustries([]); setSelectedRegion(""); setSelectedProvince(""); setSelectedCity(""); setSelectedBarangay(""); };
