  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "6px 12px 10px" }}>
        <p style={{ fontSize: "0.78rem", fontWeight: "bold", color: darkRed, marginBottom: "6px" }}>Status:</p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
          {STATUS_OPTIONS.map(s => (
            <span key={s} onClick={() => toggleStatus(s)} style={{ padding: "3px 9px", borderRadius: "20px", fontSize: "0.72rem", cursor: "pointer", userSelect: "none", background: filters.status === s ? red : "#f0e0e0", color: filters.status === s ? "white" : darkRed, border: `1px solid ${red}`, transition: "all 0.15s" }}>
              {s}
            </span>
          ))}
        </div>
...
const CompanyApplicantsScreen = ({ embedded = false, onNavigateToMessages, user, openApplicantId, onApplicantOpened, initialStatusFilter, onStatusFilterApplied }) => {
  ...
  const activeFilterCount = [filters.sex, filters.college, filters.program, filters.specialization, filters.status].filter(Boolean).length;
  ...
  const contentArea = (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {/* ── Top bar with search ── */}
      <div className="ca-topbar" style={{ justifyContent: "space-between" }}>
        <h2 style={{ fontFamily: "'Kufam', sans-serif", fontSize: "1.3rem", fontWeight: 700, color: "white", margin: 0 }}>Applicants</h2>
      ...
      <div style={{ 
        ...
        {["All", "Accepted", "Declined", "Pending", "In Review", "To Interview"].map((statusOption) => {
          ...
          const statusColor = statusOption === "All" ? "#666" : STATUS_COLORS[statusOption]?.bg || "#999";
