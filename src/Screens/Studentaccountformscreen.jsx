          ...
          <div style={rowStyle}>
            ...
            {editing ? (
              <select
                ...
                style={selectStyle}
              >
                <option value="" style={{ color: "#333" }}>N/A</option>
                {majorOptions.map(m => <option key={m} value={m} style={{ color: "#333" }}>{m}</option>)}
              </select>
            ) : (
              <span style={valueStyle}>{form.specialization || "N/A"}</span>
            )}
          </div>
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "#f5f5f5" }}>
      ...
      <div className="sap-body">
        ...
        <button
          onClick={async () => { if (window.confirm("Are you sure you want to log out?")) { await signOut(getAuth()); if (onLogout) onLogout(); } }}
          style={{
            background: "#320000",
            color: "white",
            border: "none",
