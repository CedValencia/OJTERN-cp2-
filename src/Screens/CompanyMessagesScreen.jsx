...
const ReportSuccessModal = ({ onClose }) => (
  <div className="msg-modal msg-overlay" style={{ position: "fixed", inset: 0, background: "rgba(10,10,10,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 4000, padding: space.md }}>
    <div className="msg-dialog" style={{ background: surface, borderRadius: radius.card, padding: `${space.lg} ${space.lg}`, textAlign: "center", maxWidth: "360px", width: "100%", border: `1px solid ${line}`, boxShadow: shadow.panel }}>
      <div style={{ width: "56px", height: "56px", borderRadius: "50%", background: lineSoft, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={ink} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
          <polyline points="20 6 9 17 4 12" />
        </svg>
      </div>
      <h3 style={{ fontFamily: font.ui, fontSize: "1.0625rem", fontWeight: 600, letterSpacing: "-0.01em", color: ink, margin: "0 0 6px" }}>Report submitted</h3>
      <p style={{ fontFamily: font.ui, ...type.helper, color: inkMuted, margin: "0 0 20px" }}>Thank you for reporting. Our team will review it shortly.</p>
      <button
        onClick={onClose}
        style={{ background: panel, color: onPanel, border: "none", borderRadius: radius.pill, padding: "10px 28px", fontFamily: font.ui, ...type.control, cursor: "pointer" }}
      >
        Okay
      </button>
    </div>
...
const ReportModal = ({ company, onClose, onSubmit }) => {
  ...
  return (
    <div className="msg-modal msg-overlay" style={{ position: "fixed", inset: 0, background: "rgba(10,10,10,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000, padding: space.md }}>
      <div className="msg-dialog" style={{ background: surface, borderRadius: radius.card, width: "100%", maxWidth: "520px", maxHeight: isMobile ? "92vh" : "85vh", overflow: "hidden", display: "flex", flexDirection: "column", border: `1px solid ${line}`, boxShadow: shadow.panel }}>
        <div style={{ padding: isMobile ? "14px 18px" : "16px 22px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${line}`, flexShrink: 0 }}>
          <span style={{ fontFamily: font.ui, fontSize: "1.0625rem", fontWeight: 600, letterSpacing: "-0.01em", color: ink }}>Report</span>
          <button onClick={onClose} aria-label="Close" style={{ background: "none", border: "none", fontSize: "1.1rem", cursor: "pointer", color: inkMuted, lineHeight: 1 }}>✕</button>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: isMobile ? "14px 18px" : "18px 22px" }}>
            ...
            <>
              ...
              {reportCategories.map((cat) => (
                <div key={cat.label} onClick={() => setSelected(cat)} style={{ display: "flex", alignItems: "center", gap: space.sm, padding: "11px 4px", cursor: "pointer", borderBottom: `1px solid ${lineSoft}` }}>
                  <div style={{ width: "20px", height: "20px", borderRadius: "50%", border: `2px solid ${selected?.label === cat.label ? panel : color.wine400}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, background: selected?.label === cat.label ? panel : "transparent" }}>
                    {selected?.label === cat.label && <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: onPanel }} />}
                  </div>
          ...
          {step === 2 && cat && (
            <>
              ...
              <p style={{ fontFamily: font.ui, ...type.body, color: inkBody, lineHeight: 1.7, marginBottom: "14px" }}>{cat.description}</p>
              {cat.details.length > 0 && (
                <>
                  <p style={{ fontFamily: font.ui, ...type.label, color: ink, marginBottom: "8px" }}>Common types</p>
                  <ul style={{ paddingLeft: "18px", margin: 0 }}>
                    {cat.details.map((d, i) => <li key={i} style={{ fontFamily: font.ui, ...type.helper, color: inkMuted, marginBottom: "4px" }}>{d}</li>)}
                  </ul>
                </>
              )}
            </>
          ...
          {step === 3 && (
            <>
              ...
              {!attachedFile ? (
                <div onClick={() => fileRef.current.click()} style={{ width: "72px", height: "72px", background: lineSoft, border: `1px dashed ${color.wine400}`, borderRadius: radius.card, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={inkMuted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>
                </div>
              ) : (
                <div style={{ display: "flex", alignItems: "center", gap: space.sm, background: lineSoft, border: `1px solid ${line}`, padding: "10px 14px", borderRadius: radius.card }}>
                  {attachedFile.type.startsWith("image/") ? <img src={attachedFile.url} alt="preview" style={{ width: "44px", height: "44px", objectFit: "cover", borderRadius: "8px" }} /> : <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={inkMuted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>}
                  <span style={{ fontFamily: font.ui, ...type.helper, color: inkBody }}>{attachedFile.name}</span>
      ...
      <div className="msg-list msg-list-wrapper" style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0, overflowY: "auto", background: page }}>
        ...
        {activeContacts.length === 0 ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "72px 24px", gap: space.sm, textAlign: "center", background: surface, border: `1px dashed ${color.wine400}`, borderRadius: radius.panel }}>
            ...
            <p style={{ fontFamily: font.ui, ...type.helper, color: inkMuted, maxWidth: "44ch", margin: 0 }}>Message a student from their application to start a conversation.</p>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "72px 24px", gap: space.sm, textAlign: "center", background: surface, border: `1px dashed ${color.wine400}`, borderRadius: radius.panel }}>
...
const CompanyMessagesScreen = ({
  ...
  const handleSend = async (convId, msgOrAction) => {
    ...
    else await sendMessage(convId, { text: msgOrAction.text, attachments: msgOrAction.attachments, replyTo: msgOrAction.replyTo || null });
  };
